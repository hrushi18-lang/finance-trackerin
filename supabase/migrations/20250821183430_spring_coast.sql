/*
  ...
  2. Enhanced Tables
    ...
    - `recurring_transactions` - Enhanced with bill features
    - `liabilities` - Enhanced with payment tracking
    - `goals` - Enhanced with account linking
    - `budgets` - Enhanced with account specificity
...
-- Create policies for financial_accounts (using DO blocks to prevent duplicates)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create category_budgets table for account-specific budgets
...
-- Enhance recurring_transactions with bill features
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- Create indexes for new recurring_transactions columns
...
-- Enhance liabilities with payment tracking
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_financial_accounts_updated_at'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_category_budgets_updated_at'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_bill_reminders_updated_at'
  ) THEN
...
-- Create function to update account balance when transactions are added
CREATE OR REPLACE FUNCTION update_account_balance()
RETURNS TRIGGER AS $$
BEGIN
  -- Only update balance if transaction affects balance and has an account
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_account_balance_trigger'
  ) THEN
